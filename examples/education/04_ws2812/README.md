...
Custom R2P2 firmware containing `ws2812-plus` is required. Build and flash it
with the repository-root `Mrbgems` and `Mrbgems.lock` files.
...
Check the WS2812B module power requirements. A 5 V-only module may require a
level shifter.
...
Each press prints `color 1/7` through `color 7/7` and changes the LED color.
The example succeeds when `ws2812: OK` is printed.
