...
`board`が`pico2_w`のカスタムR2P2ファームウェアが必要です。Pico 2用のUF2では
無線LANは使えません。
...
`wifi: connected (LINK_UP)`、オンボードLEDの3回点滅、最後に`wifi: OK`が表示されれば成功です。
接続できない場合はSSID・パスワード・2.4 GHz帯・国コードを確認してください。
接続待ち時間はプログラム内で15秒に設定しています。
