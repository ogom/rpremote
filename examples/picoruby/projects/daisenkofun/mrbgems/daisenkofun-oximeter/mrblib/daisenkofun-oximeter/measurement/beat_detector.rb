    ...
    module Measurement
      class BeatDetector
        ...
        def process_sample(ir:, timestamp_ms:)
          ...
          if @baseline.count < @baseline_samples || timestamp_ms - @finger_detected_at < @stabilize_ms
            @last_beat_at = timestamp_ms
            return
          end
          ...
          @last_beat_at = timestamp_ms
          { interval_ms: interval_ms, accepted: interval_ms > @min_interval_ms && interval_ms < @max_interval_ms }
        end
