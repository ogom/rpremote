  ...
  module Oximeter
    class Runner
      ...
      def initialize(
        ...
        clock: Daisenkofun::Runtime::Clock.new,
        logger: Daisenkofun::Runtime::ConsoleLogger.new,
        processor: nil,
        ...
        @device = device || Device.new(sensor: sensor, sensor_factory: sensor_factory, logger: logger)
        @clock = clock
        @logger = logger
        @processor = processor
      ...
      def start
        ...
        begin
          ...
          @device.open
          @processor ||= Measurement::Processor.new(dispatcher: @dispatcher, logger: @logger)
          @started_at = @clock.millis
          @started = true
          @running = true
          @logger.puts("DAISENKOFUN component=oximeter event=measurement_start address=0x57 duration_ms=#{@duration_ms}")
          @logger.puts("DAISENKOFUN component=oximeter event=prompt message=place_fingertip")
          self
        rescue => error
          begin
            @status_renderer.error
            @logger.puts("DAISENKOFUN component=oximeter event=error error=#{error.class} message=#{error.message}")
      ...
      def tick(timestamp_ms = nil)
        ...
        while remaining > 0
          sample = sensor.read
          @processor.process_sample(red: sample[:red], ir: sample[:ir], timestamp_ms: timestamp_ms)
          @last_tick_samples += 1
          remaining -= 1
        end
      ...
      def record_sample_backlog(available)
        ...
        @logger.puts("DAISENKOFUN component=oximeter event=fifo_backlog samples=#{available} per_tick=#{@max_samples_per_tick}")
      end

      def log_completion
        values = result
        @logger.puts(sprintf("DAISENKOFUN component=oximeter event=measurement_done bpm=%.1f spo2=%.1f", values[:bpm], values[:spo2]))
