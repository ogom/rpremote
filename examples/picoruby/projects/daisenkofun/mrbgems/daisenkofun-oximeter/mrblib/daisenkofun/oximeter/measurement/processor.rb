      ...
      class Processor
        ...
        def process_sample(red:, ir:, timestamp_ms:)
          ...
          unless beat[:accepted]
            log(
              "DAISENKOFUN component=oximeter event=beat_skipped " \
              "timestamp_ms=#{timestamp_ms} interval_ms=#{beat[:interval_ms]}"
            )
            return self
          end
          ...
          unless spo2
            log(
              "DAISENKOFUN component=oximeter event=beat_buffering " \
              "timestamp_ms=#{timestamp_ms} " \
          ...
          log(sprintf(
            "DAISENKOFUN component=oximeter event=measurement_updated " \
            "timestamp_ms=%d red=%d ir=%d bpm=%.1f spo2=%.1f state=%s",
        ...
        def update_finger(ir, timestamp_ms)
          ...
          if transition == :removed
            reset_measurement
            log(
              "DAISENKOFUN component=oximeter event=finger_removed " \
              "timestamp_ms=#{timestamp_ms} ir=#{ir}"
            )
            ...
            log(
              "DAISENKOFUN component=oximeter event=finger_detected " \
              "timestamp_ms=#{timestamp_ms} ir=#{ir}"
            )
        ...
        def log_wait(red, ir, timestamp_ms)
          ...
          log(
            "DAISENKOFUN component=oximeter event=wait " \
            "timestamp_ms=#{timestamp_ms} red=#{red} ir=#{ir}"
          )
