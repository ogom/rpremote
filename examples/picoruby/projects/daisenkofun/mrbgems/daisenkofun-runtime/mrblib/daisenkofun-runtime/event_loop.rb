...
module Daisenkofun
  module Runtime
    ...
    def initialize(publisher:, components: [], clock:, logger:, mode:, interval_ms: DEFAULT_INTERVAL_MS, warning_ms: DEFAULT_WARNING_MS)
      @publisher = publisher
    ...
    def record_tick_duration(started_at)
      ...
      @logger.puts("DAISENKOFUN mode=#{@mode} event=loop_warning tick_ms=#{elapsed} warning_ms=#{@warning_ms}")
    end
    end
  end
end
