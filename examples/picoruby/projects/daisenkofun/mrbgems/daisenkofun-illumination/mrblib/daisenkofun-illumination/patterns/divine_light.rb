      ...
      class DivineLight < Base
        def call
          each_loop do
            ...
            # 中堤上の2基は補色で同時に点灯します。
            @display.attached(Daisenkofun::Illumination::Color.rainbow(0), Config::MAX_FULL_SCENE_LEVEL, Config::MAX_FULL_SCENE_LEVEL)
            @display.set(LedLayout::DAIANJIYAMA, Daisenkofun::Illumination::Color.rainbow(3), Config::MAX_FULL_SCENE_LEVEL)
            show_frame
          end
        end
        ...
        def light_sides(outline_index)
          ...
          while position < count
            if position < left.length
              @display.set(
                left[position],
                Daisenkofun::Illumination::Color.rainbow(position * Daisenkofun::Illumination::Color::RAINBOW.length / left.length),
            ...
            if position < right.length
              @display.set(
                right[position],
                Daisenkofun::Illumination::Color.rainbow(position * Daisenkofun::Illumination::Color::RAINBOW.length / right.length),
                Config::MAX_FULL_SCENE_LEVEL
              )
            end
            show_frame
        ...
        def light_circle(outline_index)
          ...
          position = 0
          while position < half
            color = Daisenkofun::Illumination::Color.rainbow(position * Daisenkofun::Illumination::Color::RAINBOW.length / half)
