...
| `:combined` | Run measurement, status LEDs, beat-synchronized illumination, and musical behavior together. | `duration_ms`, `buzzer_pin`, `buzzer_volume`, `musical_style` |
...
| `buzzer_volume` | `3`; a 0–100 master level, with `0` selecting silent output | Combined mode |
| `musical_style` | `:heartbeat_signature` | Combined mode |

`buzzer_volume` must be a number from 0 through 100. Its default of `3` preserves the previous output level.
...
config = Daisenkofun::Application::Config.new(
  ...
  buzzer_volume: 3, # 0 also disables audio
  musical_style: :heartbeat_signature # :pulse_translation or :kofun_canon
)
```

Detected beats trigger the 572-LED biometric illumination and, unless `buzzer_pin` is `nil` or `buzzer_volume` is zero, the PWM buzzer on GP18. `buzzer_volume` is a master level: biometric duty in the 2–6% range is multiplied by `buzzer_volume / 3` and capped at 50%, where square-wave amplitude is greatest. Silent output and `:pulse_translation` use `Illumination::Biometrics::BeatPulse`; `:kofun_canon` and `:heartbeat_signature` use `Illumination::Biometrics::MoatCanon` with the same planner instance as the audio output. A successful run ends with `DAISENKOFUN mode=combined event=done status=ok`.
